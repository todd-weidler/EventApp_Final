...
function UserHostedEventsTable() {
  ...
  const [
    ...
    handleOnEnter,
  ] = useTableSearch(joinedEvents, filterByAny, "searchBar");
  ...
  const handleDeleteEvent = (name) => {
    let updatedList = [];
    let eventIndex = joinedEvents.findIndex((ev) => ev.eventName === name);

    if (eventIndex !== -1) {
      // api calls

      if (eventIndex === 0) {
        updatedList = updatedList.concat(joinedEvents.slice(1));
      } else {
        updatedList = updatedList.concat(
          joinedEvents.slice(0, eventIndex),
          joinedEvents.slice(eventIndex + 1)
        );
      }

      setJoinedEvents(updatedList);
    } else {
...
export default function UserJoinedEventsTab() {
